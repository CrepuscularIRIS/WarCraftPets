...
@register_handler(234)
class H_Prop234_DmgIfLast:
    """Damage that only applies if the user attacks last this round.

    DB2 ParamLabel: Points,Accuracy,,,,

    Typical use:
      - Tail Sweep (bonus damage if user strikes last).
      - Steady Snap (extra hit if user attacks last).
    ...
    def apply(self, ctx, actor, target, effect_row, args) -> EffectResult:
        if not _guess_attacks_last(ctx, actor, target):
            ctx.log.effect_result(effect_row, actor, target, code="SKIP_ORDER", reason="NOT_LAST")
            return EffectResult(executed=False, notes={"order": "not_last"})
        ...
        return EffectResult(executed=True, spawned_damage_events=[dmg_event], notes={"order": "last"})
