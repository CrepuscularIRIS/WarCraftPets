...
@register_handler(26)
class H_Prop26_AuraApplyDuration:
    PROP_ID = 26
    ...
    def apply(self, ctx, actor, target, effect_row, args) -> EffectResult:
        ...
        if sid:
            ...
            if eff_duration <= 0:
                ...
                return EffectResult(executed=False, notes={"resilient": int(resilient)})
        ...
        if ar.refreshed and ar.aura is not None:
            ctx.log.aura_refresh(effect_row, actor, target, int(aura_id), int(ar.aura.remaining_duration), tdf)

        ctx.log.aura_apply(effect_row, actor, target, int(aura_id), int(eff_duration), tdf, ar.reason)
        ...
        if stats is not None and hasattr(stats, "sync_pet"):
            try:
                ...
                pass
        return EffectResult(executed=(ar.applied or ar.refreshed), notes={"aura_reason": ar.reason, "duration": int(eff_duration)})
